...
class Experiment(object) :
    ...
    def run_experiment(self, verb=True) :
        ...
        instructions = """
        At each trial you see one fixation cross,
        then a round target disk. Move your eyes
        towards the target. If the screen flickers, blink
        ...
        text = visual.TextStim(win, text='Ready?', alignHoriz='center', alignVert='center', color='black')
        ...
        # EXPERIMENT
        # ---------------------------------------------------
        for i_trial, trial in enumerate(self.param_exp['trials']) :

            # ---------------------------------------------------
            # FIXATION
            # ---------------------------------------------------
            if (i_trial % (self.args.N_trials//6)) == 0:
                text.setText(f"{i_trial:03d}/{self.args.N_trials:03d}")
                text.draw()
            else:
                fixation.draw()
            possible_escape()
            ...
            try:
                filename = os.path.join(self.datadir, trial, self.timeStr[:10] + '_' + self.args.observer + '__' + '%.3d' % i_trial + '.png')
                imageio.imwrite(filename, frame[:, :, ::-1]) # converting on the fly from BGR > RGB
            except Exception as e: #else :
                print(f"trial {i_trial} : non enregistré - error {e}")
                msg_Warning = visual.TextStim(win, text=u"\n\n\ntrial %.3d : non enregistré"%i_trial,
